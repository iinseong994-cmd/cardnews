...
SCHEMA = """{
  ...
  "slides": [
    ...
    {"no":2,"type":"photo","eyebrow":"<각도 한 단어>","headline":"<제목, **강조** 가능>",
     "subhead":"<이 사진이 무엇을 보여주는지 한 줄>","image_path":"<이 문구를 뒷받침하는 사진>"},
...
8. **분량을 채운다.** 카드가 비어 보이면 안 된다.
   ...
   - 사진이 넉넉하면 **2·3·4·6번을 모두 photo** 로 채워라. 글자만 있는 카드가 이어지면 허전하다
...
16. **사진은 그 카드의 문구와 맞는 것을 골라라.** 남는 걸 아무거나 끼우지 마라.
    - 착용감 얘기면 사람이 쓰고 있는 컷, 구성품 얘기면 제품이 다 보이는 컷
    - `subhead` 에 **그 사진에서 무엇을 보라는 건지** 한 줄 적어라
      (예: "케이스를 닫아도 안이 비칩니다" — 사진과 문구가 따로 놀면 안 된다)
    - 같은 사진을 두 카드에 쓰지 마라
17. **JSON만 출력한다.** 설명·마크다운 코드펜스 없이 `{` 로 시작해서 `}` 로 끝낸다.
"""
